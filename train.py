...
class Trainer(object):
    ...
    def __init__(self, train_data, valid_data, input_dim, output_dim=None):
        ...
        self.memory_size = (self.episode_length * self.batch_size if FLAGS.memory_size is None else FLAGS.memory_size)
        # use locality-sensitive hashing
        self.use_lsh = FLAGS.use_lsh
        self.output_dim = (output_dim if output_dim is not None else self.episode_width)
    ...
    def get_model(self):
        ...
        return model.Model(self.input_dim, self.output_dim, self.rep_dim, self.memory_size, vocab_size, use_lsh=self.use_lsh)
    ...
    def run(self):
        ...
        for i in range(FLAGS.num_episodes):
            ...
            if i % FLAGS.validation_frequency == 0:
                ...
                logging.info('%d-shot: %.3f, ' * (self.episode_width + 1), *sum([[k, np.mean(correct_by_shot[k])]
