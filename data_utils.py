...
def get_data():
    ...
    for data, processed_data in zip([train_data, test_data], [processed_train_data, processed_test_data]):
    ...
    for root, _, files in os.walk(directory):
        ...
        for file_name in files:
            ...
            for i, angle in enumerate([0, 90, 180, 270]):
                ...
                # 这里可以看到，对图片进行旋转的时候也对索引进行了处理，这里可以认为是经过旋转后的图像也属于
                # 不同的类别了，所以在一定程度上算是丰富了数据集
...
def preprocess_omniglot():
    ...
    num_labels = write_datafiles(directory,
                        write_file,
                        resize=True,
                        rotate=TRAIN_ROTATIONS,
                        new_width=IMAGE_NEW_SIZE,
                        new_height=IMAGE_NEW_SIZE)
